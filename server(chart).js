...
app.post('/sold', (req, res) => {
  ...
  history.push({ action: 'sold', player: auctionState.name, team: auctionState.soldTeam, finalPrice: auctionState.finalPrice, timestamp: new Date().toISOString() });
  fs.writeFileSync(path.join(__dirname, 'data', 'history.json'), JSON.stringify(history, null, 2));
  io.emit('update', auctionState);
  res.json({ success: true });
});
...
app.post('/undo', (req, res) => {
  ...
  if (action === 'sold') {
    ...
    if (teamRoster.indexOf(player) !== -1) teamRoster.splice(teamRoster.indexOf(player), 1);
